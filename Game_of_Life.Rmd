...
These are some patterns that will repeat itself after a certain number of iterations. They stay in place and do not move around the map.
...
These are many patterns that will move across the screen. 4 of the most common inistial states will be shown below.
...
Guns are patterns that will shot things out! Essentially they generate spaceships, most commonly the ***glider*** pattern.
...
The *first* discovered gun is Gosper glider gun. This is the first pattern that is known to be able to grow till infinity. The smallest glider gun that exists is the Simkin glider gun. It only consists of 36 cells.
...
list.files(path = path, pattern = "*.png", full.names = T) %>%
  image_read() %>%
  image_join() %>% 
  image_animate(fps=10, delay = 5) %>% 
  image_write(paste0(path,"/",gif_name,".gif", collapse = ""))
}

# GoL(mat, "/Volumes/MyBook_3/git/R-random-spontaneous-projects/Game_of_Life_files/puffers/puffer2/",200) 
make_gif("/Volumes/MyBook_3/git/R-random-spontaneous-projects/Game_of_Life_files/puffers/puffer2/", "puffer2")
```

These are some of the most common patterns in Conway's game of life. There are many more to it. 

Here is a [link](https://conwaylife.com/wiki/Main_Page) for those who want to learn more about this simple yet complex "game".