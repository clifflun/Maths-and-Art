...
save <- function(mat, path, iter){
  png(paste0(path,sprintf("%03d",iter), ".png", collapse = ""))
...
list.files(path = path, pattern = "*.png", full.names = T) %>%
  image_read() %>%
  image_join() %>% 
  image_animate(fps=10, delay = 35) %>% 
  image_write(paste0(path,"/",gif_name,".gif", collapse = ""))
}
```

```{r eval=FALSE, include=FALSE}
GoL(mat, "/Volumes/MyBook_3/git/R-random-spontaneous-projects/Game_of_Life_files/pulsar/",2)
make_gif("/Volumes/MyBook_3/git/R-random-spontaneous-projects/Game_of_Life_files/pulsar/", "pulsar")
```
